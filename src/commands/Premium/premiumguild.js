...
const authorizedOwners = ["1085459608707014706", "1379911892037800128"];
...
module.exports = {
  ...
  run: async (client, message, args, prefix) => {
    ...
    if (!guildId) {
      return message.reply(" Please specify a guild ID!");
    }
    ...
    try {
      ...
      if (me && me.manageable) {
        await me.setNickname("Pookie").then(() => {
          console.log(`Nickname changed to "pookie" in ${guild.name}`);
        }).catch((err) => {
